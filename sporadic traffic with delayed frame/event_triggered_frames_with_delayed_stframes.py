                ...
                if offline_schedule[flag_id + 1].start_time - sporadic_arrive[j] == 0:
                    if flag_id + 1 != delayed_sche_id:
                        ...
                        if offline_schedule[flag_id + 1].source != preemptable_flow or \
                            ...
                            print(
                                "sporadic frame is delayed and keep in the current loop 2.8 from flag 3, not delayed")
                            print(sporadic_arrive)
                            print(sporadic_C)
                            print(mark)
                        else:
                        ...
                        if sporadic_arrive[j] + C_CBS_remain - offline_schedule[flag_id + 1].end_time > 2:
                            ...
                            sporadic_C[j] = sporadic_C[j] - (
                                    offline_schedule[flag_id + 1].end_time - sporadic_arrive[j] + 1) + 0.3
                            C_CBS_remain -= C_CBS_remain - (
                                    offline_schedule[flag_id + 1].end_time - sporadic_arrive[j] + 1) + 0.3
                            sporadic_arrive[j] = offline_schedule[flag_id + 1].end_time
                            print(
...
def frame_prioritization(j, offline_schedule, sporadic_response_time, delayed_response_time, prioritized_id,
    ...
    if sporadic_arrive[j] == sporadic_arrive[prioritized_id]:
        ...
        temp_arrive_backpack = sporadic_arrive_backpack[prioritized_id]
        ...
        sporadic_arrive_backpack.pop(prioritized_id)
        ...
        sporadic_arrive_backpack.insert(j, temp_arrive_backpack)
        ...
        if offline_count == 0:
            print("no interference from ST frames with parameters: ", sporadic_C[j], sporadic_arrive[prioritized_id], sporadic_arrive[j])
...
def sporadic_frame_response_time(j, sporadic_c, sporadic_arrive_t, offline_schedule, deadline_U_CBS, C_CBS_remain,
                                 ...
                                 sporadic_interval, Uti_CBS, sporadic_arrive_backpack):
    # interference_sporadic = 0
        ...
        if delay_count_1 > 0:
            ...
            if delayed_release_time <= sporadic_arrive[j] <= delayed_response_time:
                ...
                if sporadic_response_time - delayed_release_time < 2:
                    ...
                    sporadic_arrive_backpack.insert(j + 1, delayed_release_time)
        ...
        if j < len(sporadic_arrive_time_AVB) - 1:
            ...
            if index != -5:
                ...
                if sporadic_arrive_time_AVB[j] > sporadic_arrive_time_AVB[index] and mark_credit < 0:
                    ...
                    # if sporadic_arrive_time_AVB[j] > original_arrive:
                    if response_time > sporadic_arrive_time_AVB[index]:
                        sporadic_arrive_time_AVB[index] = response_time
...
if __name__ == "__main__":
    ...
    AVB_min_response_time_classA = []
    AVB_min_response_time_classB = []
            ...
            if j < len(sporadic_arrive) and mark[j] != 1000000:
                ...
                deadline_U_CBS, sched_check, delayed_response_time, error, delayed_error, delay_count_1 = \
                    sporadic_frame_response_time(j, sporadic_C[j], sporadic_arrive[j], offline_schedule, deadline_U_CBS,
                                                 ...
                                                 retransmiss_st_deadline,
                                                 sched_check, sporadic_interval, Uti_CBS, sporadic_arrive_backpack)
    ...
    # print(CBS_min_response_time_classA)
    ...
    plt.title("The finish time of critical frames from Class A, Uti: 0.3 with 10 ST flow", fontsize = 12, pad=10)
    ...
    plt.title("The finish time of critical frames from Class B, Uti: 0.3 with 10 ST flow", fontsize = 12, pad=10)
    # plt.tick_params(labelsize=22)
    plt.legend(fontsize = 12)
    plt.show()
    #
    ...
    # bplot = plt.boxplot([difference_CBS_AVB_classA, difference_CBS_AVB_classB], labels=labels, meanline=True,
    #                     showmeans=True)
    # plt.show()
